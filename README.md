...
- 現状は W1 (バックエンド土台+認証) まで完了。todo/notes等のデータAPI・AI連携・ファイルストレージは今後のフェーズ (W2〜W6) で追加する
- **セットアップ**: `.env.example` を `.env.local` にコピーして `DATABASE_URL` (Neonのダッシュボードで発行) と `JWT_SECRET` を設定してから `npm run dev:web` (`vercel dev`) で起動する。Vercelアカウント作成・プロジェクトのリンクはユーザー自身の操作が必要
...
- [ ] W2: コアデータAPI (todo/notes/recent/usage/plugins)
- [ ] W3: renderer アダプタ層 (`window.api` の Web実装) + ログイン画面 + Webビルド
